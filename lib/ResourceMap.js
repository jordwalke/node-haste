...
/**
 * @param {Array<string>} types List or potential resource types.
 * @param {string} id Module ID.
 * @return {Resource} The first resource found by searching for type, id.
 */
ResourceMap.prototype.getFirstResource = function(types, id) {
  var dependencyResource = null;
  for (var j = 0; j < types.length; j++) {
    dependencyResource = this.getResource(types[j], id);
    // Stop at the first one we see.
...
ResourceMap.prototype.removeResource = function(resource) {
  console.log('removint resource:', resource.id);
  var type = this.typeToMap[resource.type] || resource.type;
  this.configurationTrie = this.resourceCache = null;
  this.inferredProjectPaths = null;
