...
/**
 * @param {array<ResourceLoader.prototype>} loaders List of resource loaders.
 * Pass the array in order of priority - to resolve conflicts (meaning if
 * `file.js` and `file.css` exist, `require('file')` defaults to js) if you pass
 * [JSLoader, CSSLoader].
 * @return {array<string>} List of all combined, deduped extensions.
 */
ResourceLoader.getExtensionsOfLoaderClasses = function(loaderClasses) {
  var ext = {};
  loaderClasses.forEach(function(loader) {
    loader.prototype.getExtensions().forEach(function(e) {
      ext[e] = true;
    });
  });
...
/**
 * @param {array<ResourceLoader.prototype>} loaderClasses List of Loaders.
 * @return {array<Class>} List of all combined resource classes.
 */
ResourceLoader.getResourceClassesOfLoaderClasses = function(loaderClasses) {
  var resourceClasses = [];
  loaderClasses.forEach(function(loader) {
    resourceClasses = resourceClasses.concat(
      loader.prototype.getResourceTypes()
    );
  });
...
/**
 * Super confusing that each loader has a getResourceTypes (list of `Resource`
 * classes) and each `Resource` has a `.prototype.type` which is a string.
 * @param {array<ResourceLoader.prototype>} loaderClasses List of Loaders.
 * @return {array<string>} List of all combined resource types.
 */
ResourceLoader.getTypesOfLoaderClasses = function(loaderClasses) {
  var resourceTypes = [];
  ResourceLoader.getResourceClassesOfLoaderClasses(loaderClasses)
    .forEach(function(resourceClass) {
      resourceTypes = resourceTypes.concat(resourceClass.prototype.type);
    });
