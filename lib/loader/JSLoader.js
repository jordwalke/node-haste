...
/**
 * Default list of valid extensions that should be searched for when a JS module
 * does `require('x')`. `x` might not have a file extension, so we need a list
 * of what is valid to require. Pure node has the concept of
 * `require._extension` - this is similar - but only lists what `node-haste`
 * should build into the `ResourceMap`. All interpretation of these required
 * files is deferred until packaging/analyzing/etc. Customize this set of valid
 * extensions by providing the `this.config.validJSModuleDependencyLoaders`
 * option to the `JSLoader` constructor. They are prioritized because
 * `PathResolver` will stop once it finds one that matches - provide them in the
 * right order.  Note: This is *not* to say you must `require('file.js')`, it is
 * to say that when you require something without a clear extension, which files
 * are searched.
 */
var defaultValidJSModuleDependencyLoaders = [JSLoader];
...
function JSLoader(options) {
  ...
  var validJSModuleDependencyLoaders =
    options.validJSModuleDependencyLoaders ||
    defaultValidJSModuleDependencyLoaders;

  this.validJSModuleDependencyLoaders = validJSModuleDependencyLoaders;
  // Cache this computation - all the valid extensions that a JS module could
  // possibly depend on via require().
  this._computedRequireSearchesExtensions =
    ResourceLoader.getExtensionsOfLoaderClasses(validJSModuleDependencyLoaders);
  // Cache this computation - all the valid Resource types that a JS module
  // could possibly depend on via require().
  this._computedRequireableResourceTypes = ResourceLoader
    .getResourceClassesOfLoaderClasses(validJSModuleDependencyLoaders)
      .map(function(resourceClass) {
        return resourceClass.prototype.type;
      });
...
JSLoader.prototype.loadFromSource =
  ...
  console.log('loaded JS with ID:', js.id);
};
 ...
 */
function findAbsolutePathForRequired(requiredText, callersPath, resourceMap, searchExtensions) {
  var callerData =  {
    id: callersPath,
    paths: resourceMap.getAllInferredProjectPaths(),
    fileName: callersPath
  };
  return PathResolver._resolveFileName(requiredText, callerData, resourceMap, searchExtensions);
}
...
JSLoader.prototype.postProcess = function(map, resources, callback) {
  ...
  resources.forEach(function(r) {
    ...
    for (var i = 0; i < required.length; i++) {
      ...
      var resourceByID = map.getFirstResource(
        this._computedRequireableResourceTypes,
        requiredText
      );

      // This works out because of our particular id scheme.
      ...
      var commonJSResolvedPath = beginsWithDot && textInCache ?
        nonRelativePathCache[requiredText] : findAbsolutePathForRequired(
          requiredText,
          r.path,
          map,
          this._computedRequireSearchesExtensions
