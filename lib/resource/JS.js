 ...
 *   }
 *
 *   requiredModules: ['package-name/path/comp.css', 'package-name/path/comp.js']
 *
 *   _requiredTextToResolvedID: {
 *     'JS': {''./path/comp.js': 'package-name/path/comp.js'}
 *     'CSS': {''./path/comp.css': 'package-name/path/comp.css'}
 *   }
 *
 * @param {string} origName String passsed to require() in js file.
 * @param {string} type Type of module that required `origName` resolves to from
 * the perspective of this particular resource.
 * @param {string} modID moduleID that `origName` resolves to from the
 * perspective of this particular resource.
 */
JS.prototype.recordRequiredModuleOrigin = function(type, origName, modID) {
  if (!this._requiredTextToResolvedID[type]) {
    this._requiredTextToResolvedID[type] = {};
  }
  this._requiredTextToResolvedID[type][origName] = modID;
};
 ...
 */
JS.prototype.getModuleIDByOrigin = function(type, origName) {
  return (
    this._requiredTextToResolvedID[type] &&
    this._requiredTextToResolvedID[type][origName]
  ) || origName;
};
